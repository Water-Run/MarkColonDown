...
`Mark:down`是一个强大的`Markdown`模板引擎. 作为`Markdown`的严格超集, 由`.md`编译为`.md`, 大幅扩展了`Markdown`的能力.  
...
4. **内嵌代码**. 在 `#:code` 块中执行 `neolua` 脚本进行逻辑计算或调用外部 `lua` 模块, 并通过 `{{ }}` 将表达式结果嵌入正文. 灵活强大带来了非常多的可能性.
...
`neolua`代码的内嵌可能是`Mark:down`最强大的功能, 条件判断也依赖于它. 同时, 它也很危险--`Mark:down`并不会判断你的执行的代码的安全性, 你需要自己负责.  
